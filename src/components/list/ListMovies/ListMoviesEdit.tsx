...
export const ListMoviesEdit = ({ initialMovies }: { initialMovies: Movie[] }) => {
  const keys = useListStore((state) => [...state.data.movies.keys()], shallow);

  if (!keys || !keys.length) {
    return (
      <ul className="grid grid-cols-7 gap-2 mb-44" suppressHydrationWarning={true}>
  ...
  return (
    <ul className="grid grid-cols-7 gap-2 mb-44">
      {keys.map((key) => (
